...
import { 
    ...
    ArrowUpDown, 
    Loader2,
    ArrowUp // New icon for the raise indicator
} from 'lucide-react';
...
export const Dashboard = () => {
    ...
    const { currency: selectedCurrency } = useSelector((state) => state.ui); // Get global currency
    ...
    if (employeesStatus === 'loading') {
        return (
            <div className="flex justify-center items-center h-64"><Loader2 className="animate-spin text-slate-500" size={48} /><p className="ml-4 text-lg text-slate-600">Loading Employees...</p></div>
        );
    }
            ...
            <div className="bg-white shadow-md rounded-lg overflow-hidden">
                ...
                <div className="p-4 border-b border-gray-200 flex flex-col md:flex-row items-center justify-between gap-4">
                    <div className="relative w-full md:w-1/3">
                        ...
                        <input type="text" placeholder="Search by name..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-amber-500 focus:outline-none"/>
                    </div>
                    <div className="flex items-center gap-2">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {filteredAndSortedEmployees.map((employee) => {
                                ...
                                <tr key={employee._id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    {user?.role === 'hr' && (<td className="px-6 py-4 whitespace-nowrap text-sm font-medium flex gap-4"><button onClick={() => dispatch(openAddEmployeeDrawer(employee))} className="text-slate-600 hover:text-amber-600" title="Edit Employee"><Edit size={18} /></button><button onClick={() => dispatch(openAddHolidayDialog(employee))} className="text-slate-600 hover:text-green-600" title="Add Holiday"><CalendarPlus size={18}/></button></td>)}
                                </tr>
                            )})}
                        </tbody>
                    </table>
                    ...
                    {filteredAndSortedEmployees.map(employee => {
                        ...
                        <div key={employee._id} className="bg-white border rounded-lg p-4 shadow-sm">
                            <div className="flex justify-between items-start">
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 flex-shrink-0 bg-slate-200 rounded-full flex items-center justify-center font-bold text-slate-600">{getInitials(employee.name)}</div>
                                    <div><p className="font-bold text-gray-900">{employee.name}</p><p className="text-sm text-gray-500">{employee.title}</p></div>
                                </div>
                            ...
                            <div className="space-y-2 text-sm">
                                <div className="flex justify-between"><span className="text-gray-500">Department:</span> <span className="font-medium">{employee.dept}</span></div>
                                <div className="flex justify-between items-center">
                                    ...
                                    <div className="flex items-center gap-2 font-medium">
                                        ...
                                        {mostRecentRaise && (
                                            <div className="relative group">
                                                <ArrowUp className="text-green-500" size={16} />
                                                <div className="absolute bottom-full right-0 mb-2 w-max bg-slate-800 text-white text-xs rounded-md py-1 px-3 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">Raise: {formatDynamicCurrency(mostRecentRaise.amount, mostRecentRaise.currency)} on {new Date(mostRecentRaise.date).toLocaleDateString()}</div>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                <div className="flex justify-between"><span className="text-gray-500">Days Off:</span> <span className="font-medium">{employee.holidays.length}</span></div>
                            </div>
                            {user?.role === 'hr' && (<><div className="border-t my-3"></div><div className="flex justify-end gap-4"><button onClick={() => dispatch(openAddEmployeeDrawer(employee))} className="flex items-center gap-2 text-sm text-slate-600 hover:text-amber-600"><Edit size={16} /> Edit</button><button onClick={() => dispatch(openAddHolidayDialog(employee))} className="flex items-center gap-2 text-sm text-slate-600 hover:text-green-600"><CalendarPlus size={16}/> Add Holiday</button></div></>)}
                        </div>
                    )})}
                </div>
